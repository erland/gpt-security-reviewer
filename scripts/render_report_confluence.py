...
def render(report):
 ...
 if flows:o += [table(['Flöde/attackyta','Analyserat fokus','Status','Resultat / nästa steg'],[[x['flow'],x['review_focus'],x['status'],x.get('result_next_step') or x.get('evidence_basis') or '–'] for x in flows]),'']
 ...
 if fs:
  ...
  for f in fs:
   o += [f"h3. {f['id']} – {f['title']}",'',f"*Kategori:* {f['category']}",f"*Severity:* {f['severity']}",f"*Confidence:* {f['confidence']}",f"*Status:* {f['status']}",f"*Berörda komponenter:* {', '.join(f.get('affected_components') or ([f.get('component')] if f.get('component') else [])) or 'Ej angivet'}",f"*Manuell verifiering:* {f['manual_verification']}",'','*Observation*','',f['observation'],'']
