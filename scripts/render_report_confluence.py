...
def render(report):
 m=report['metadata'];e=report['executive_summary'];s=report['scope'];sy=report['system_overview'];a=report['architecture_security'];c=report['coverage'];rr=report['residual_risk'];ap=report.get('appendix') or {};fs=sorted(report.get('findings',[]),key=lambda f:(SEVERITY_ORDER.get(f['severity'],99),f['id']))
 o=[f"h1. {m['title']}",'','h2. Metadata','',table(['Fält','Värde'],[['System/IT-stöd',m.get('system_name') or 'Ej angivet'],['Granskningsdatum',m.get('review_date') or 'Ej angivet'],['Granskningsläge',m.get('review_mode')],['Version',m.get('version')],['Underlagsreferens',m.get('source_reference') or 'Ej angivet']]),'','h2. Sammanfattning','',e['overall_assessment'],'','h3. Viktigaste fynd','',bullets(e['key_findings']),'','h3. Viktigaste osäkerheter','',bullets(e['key_uncertainties']),'','h3. Rekommenderade nästa steg','',bullets(e['next_steps']),'','h2. Scope och analyserat underlag','',s['requested_scope'],'','h3. Analyserat underlag','',bullets(s['reviewed_material']),'','h3. Avgränsningar','',bullets(s['limitations']),'','h2. System- och tekniköversikt','']
 for k,l in [('frontend','Frontend'),('backend','Backend'),('data_stores','Datalager'),('integrations','Integrationer'),('deployment','Deployment')]:o += [f'h3. {l}','',bullets(sy.get(k,[])),'']
 o += ['h2. Arkitekturell säkerhetsbild','']
