...
for x in ['canonical/report-model.md','canonical/reporting-contract.md','canonical/report-modes.md','schemas/report.schema.json','schemas/review-process.schema.json','tests/fixtures/report-standard-example.json']:
    if not (R/x).exists(): e.append('Missing: '+x)
for x in ['schemas/finding.schema.json','schemas/review-summary.schema.json','schemas/report.schema.json','schemas/review-process.schema.json','tests/fixtures/report-standard-example.json']:
    try: json.loads((R/x).read_text(encoding='utf-8'))
    except Exception as ex: e.append(f'Invalid JSON {x}: {ex}')
for tok in ['### 1. Metadata','målgruppsingång för utvecklingsteamet','målgruppsingång för säkerhetsgranskaren','### 3. Systemöversikt','### 4. Analyserade säkerhetsrelevanta flöden och attackytor','result_next_step','### 7. Fynd','affected_components','acceptance_criteria','### 8. Coverage','### 10. Rekommenderad fortsatt granskning','### 11. Kvarvarande risk','Internt review_process-objekt']:
